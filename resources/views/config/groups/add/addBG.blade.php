...
<div class="container p-4 bg-white rounded shadow mx-auto my-4 w-50">
    ...
    <form action="{{ route('createBG') }}" method="get">
        ...
        <input type="text" name="boxgroup_name" class="form-control @error('boxgroup_name') border-danger @enderror" id="bg_name" value="{{ old('boxgroup_name') }}" pattern="[a-zA-Z][a-zA-Z0-9-_+ ]{4,200}" title="HostGroup name must be between 4 & 200 charcarters in length and containes only letters, numbers, and these symbols -_+">
        @error('boxgroup_name')
