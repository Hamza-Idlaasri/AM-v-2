...
<div class="container p-3 bg-white w-50 rounded shadow mx-auto my-4">
    ...
    <form action="{{ route('editEG', $equipgroup->servicegroup_id) }}" method="get">
        ...
        <input type="text" name="equipgroup_name" class="form-control w-100 @error('equipgroup_name') border-danger @enderror" id="eg_name" value="{{ $equipgroup->equipgroup_name }}" pattern="[a-zA-Z][a-zA-Z0-9-_+ ]{2,200}" title="EquipGroup name must be between 2 & 200 charcarters in length and containes only letters, numbers, and these symbols -_+">
        ...
        <div class="p-2 bg-white w-100" style="overflow: auto;max-height:250px;border:1px solid rgb(216, 215, 215);border-radius:5px">
            @forelse ($equips as $equip)
                @if (in_array($equip->service_object_id, $all_members))
                    <input type="checkbox" name="members[]" id="{{$equip->service_object_id}}" value="{{$equip->service_object_id}}" checked> <label for="{{$equip->service_object_id}}" style="user-select: none"> {{$equip->equip_name}} <span class="text-secondary">({{$equip->box_name}})</span></label>
                    <br>
                @else
