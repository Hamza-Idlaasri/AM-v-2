...
<div class="container my-3 w-50 pt-2 pb-4">
    ...
    <form action="{{ route('save-box-edits', $box->host_object_id) }}" method="get">
        ...
        <div class="card shadow-sm">
            ...
            <div class="card-body">
                ...
                <input type="text" name="boxName" class="form-control @error('boxName') border-danger @enderror"
                    id="box_name" value="{{ $box->display_name }}" pattern="[a-zA-Z][a-zA-Z0-9-_+ ]{2,200}"
                    title="Box name must be between 4 & 200 charcarters in length and containes only letters, numbers, and these symbols (-_+)">
                @error('boxName')
                ...
                <input type="text" name="addressIP" class="form-control @error('addressIP') border-danger @enderror"
                    id="ip" minlength="7" maxlength="15" size="15"
                    pattern="^((\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.){3}(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])$"
                    value="{{ $box->address }}" title="Please enter the IP address correctly e.g. 192.168.1.1">
                @error('addressIP')
                    <div class="text-danger">
                        {{ $message }}
                    </div>
                    ...
                    <div class="sizing" style="max-height:100px; overflow: auto">
                        ...
                        @for ($i = 0; $i < sizeof($parent); $i++)
                            @if ($parent[$i]['relation'] == 'parent')
                                <input type="radio" name="boxes" id="{{ $parent[$i]['host_name'] }}"
                                    value="{{ $parent[$i]['host_name'] }}" checked> <label
                                    for="{{ $parent[$i]['host_name'] }}"> {{ $parent[$i]['host_name'] }}</label>
                                <br>
                            @endif

                            @if ($parent[$i]['relation'] == 'none')
                                <input type="radio" name="boxes" id="{{ $parent[$i]['host_name'] }}"
                                    value="{{ $parent[$i]['host_name'] }}"> <label
                                    for="{{ $parent[$i]['host_name'] }}"> {{ $parent[$i]['host_name'] }}</label>
                                <br>
                            @endif
                        @endfor
        ...
        <div class="card shadow-sm">
            ...
            <div class="card-body">
                {{-- Check Interval --}}
                <label for="CheckInterval"><b>Normal Check Interval
                        <!--<span class="text-danger">*</span>--></b></label>
                <div class="d-flex">
                    <input type="number" min="5" max="10000" name="check_interval"
                        class="form-control p-unity @error('check_interval') border-danger @enderror" id="CheckInterval"
                        value="{{ $box->normal_check_interval }}">
                ...
                <div class="d-flex">
                    <input type="number" min="5" max="10000" name="retry_interval"
                        class="form-control p-unity @error('retry_interval') border-danger @enderror" id="retryInterval"
                        value="{{ $box->retry_check_interval }}">
                    <span class="unity">sec</span>
                </div>
                ...
                <div class="d-flex">
                    <input type="number" min="1" max="100" name="max_attempts"
                        class="form-control p-unity @error('max_attempts') border-danger @enderror" id="maxInterval"
                        value="{{ $box->max_check_attempts }}">
                ...
                <label for="notifInterval"><b>Notification Interval
                        <!--<span class="text-danger">*</span>--></b></label>
                <div class="d-flex">
                    <input type="number" min="1" max="1000" name="notif_interval"
                        class="form-control p-unity @error('notif_interval') border-danger @enderror"
                        id="notifInterval" value="{{ $box->notification_interval }}">
