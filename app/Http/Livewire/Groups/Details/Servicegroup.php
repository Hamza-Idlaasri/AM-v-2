...
class Servicegroup extends Component
{
    ...
    {
        $this->servicegroup = DB::table('nagios_servicegroups')
            ->where('servicegroup_id',$request->id)
            ->get();

        $this->members = DB::table('nagios_servicegroups')
            ...
            ->where('nagios_servicegroups.servicegroup_id',$request->id)
    ...
    public function render()
    {
        return view('livewire.groups.details.servicegroup')
            ->with(['servicegroup' => $this->servicegroup,'members' => $this->members])
            ->extends('layouts.app')
            ->section('content');
    }
}
